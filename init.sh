...
if [[ "$choice" =~ ^[Yy]$ ]]; then
    ...
    run "Installing AWVS" "sudo systemctl start docker && sudo docker pull ghcr.io/xrsec/awvs:24.4.240427095 && sudo docker run -it -d --name awvs -p 127.0.0.1:3443:3443 --restart=always ghcr.io/xrsec/awvs:24.4.240427095 && echo $(hostname -I | awk '{print $2}') awvs.lan | sudo tee -a /etc/hosts"
    run "Activating AWVS" "sudo wget -O /usr/local/share/ca-certificates/RootCA.crt https://cdn.jsdelivr.net/gh/XRSec/AWVS-Update@main/.github/resources/ca.cer && sudo update-ca-certificates && sudo docker exec -i 'awvs' bash -c 'apt update -y && apt upgrade -y && apt install -y libsqlite3-dev wget && wget https://raw.githubusercontent.com/ngductung/acunetix23/main/check-tools.sh --no-check-certificate -O ./check-tools.sh && chmod +x ./check-tools.sh && ./check-tools.sh'"
    printf '%s\n' "Success!" \
    ...
    run "Installing mitmproxy" "git clone https://github.com/mitmproxy/mitmproxy.git && cd mitmproxy && curl -LsSf https://astral.sh/uv/install.sh | sh && uv run mitmproxy"
